      ...
      <main className="mx-auto max-w-7xl px-6 py-20">
        <div className="grid gap-8 xl:grid-cols-[320px_1fr]">
          <aside className="rounded-[2rem] border border-white/10 bg-[#09101f] p-8 shadow-cinematic">
            ...
            <h1 className="mt-6 text-3xl font-semibold text-white">Bookings</h1>
            <nav className="mt-12 space-y-2">
              ...
              <a href="/dashboard/admin/bookings" className="block rounded-3xl bg-gold px-4 py-3 text-ink">Bookings</a>
            ...
            <div className="rounded-[2rem] border border-white/10 bg-[#0b1321] p-8 shadow-cinematic">
              ...
              <div className="mt-8 space-y-4">
                {bookings.map((booking: Booking) => (
                  <div key={booking.id} className="rounded-3xl border border-white/10 bg-[#091524] p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-lg font-semibold text-white">{booking.name}</p>
                        <p className="text-sm text-sand/80">{booking.email}</p>
                        <p className="text-sm text-gold">{booking.service}</p>
                      </div>
                      <div className="text-right">
                        <p className={`text-sm ${booking.status === 'APPROVED' ? 'text-green-400' : booking.status === 'REJECTED' ? 'text-red-400' : 'text-yellow-400'}`}>
                          {booking.status}
                        </p>
                        <p className="text-xs text-sand/60">{booking.createdAt.toLocaleDateString()}</p>
                      </div>
                    </div>
                    <p className="mt-4 text-sand/80">{booking.message}</p>
                  </div>
                ))}
              </div>
            </div>
